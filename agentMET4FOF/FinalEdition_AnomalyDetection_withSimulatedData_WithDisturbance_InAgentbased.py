...
# Defenition of process engine of pytorch model, so if some machine does not have GPU it could be used CPU instead
...
class Decoder_withoutLSTM(nn.Module):
    ...
    def __init__(self, seq_len, input_dim=64, n_features=1):
        ...
        self.decoder_L2 = nn.Linear(in_features=self.hidden_dim, out_features=self.n_features,
...
#Time selected from Sensor1_1, so Time of Sensor2_1 and Sensor3_1 are omitted
            a = pd.DataFrame(self.memory['Sensor1_1'])
            b = pd.DataFrame(self.memory['Sensor2_1']['y1'])
            c = pd.DataFrame(self.memory['Sensor3_1']['y1'])

            #Concatenate there different sensor data and remove duplicatd time columns,
            # so there is one dataframe with one time column and three different sensor data in three columns
...
#**********************************************************************************************************************
        #Filled X_train_df in amount of train_size, so start to train model with X_train_df
    ...
    def Train_func(self,X_train_df):
        ...
        # Calculate standard deviation of train data that in next steps will be puted in threshold
        self.log_info(f'X_train_std:{X_train_std}')

        # Pass X_train_df to function create_dataset for generating list of tensors that will be read in pytorch model as input data
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            # Put state_dict of best model(means each model that has minimum loss) in best_model_wts variable
            ...
            self.log_info(f'Epoch:{self.model_type} {epoch}: train loss {np.round(train_loss, 6)}')
...
########################################################################################################################
    def Prediction_func(self,message, model):
            ...
            #Pass every stream blocks of test data(in amount of buffer size) to model for prediction
            predictions, losses = [], []
            with torch.no_grad():
                model = model.eval()
            ...
            ############################################################################################################################
            #scored dataframe consisted by all calculation's results that will be showed in monitor_agent_2 in final output web page
            scored = pd.DataFrame()
            ...
            #Calculate loss value that used mean square error formula for calculating of loss
            ...
            self.uncertainty_loss_der_square =pd.DataFrame(((X_test_arr - X_pred_arr) / 2) ** 2)
            self.log_info(f'uncertainty_loss_der_square: {self.uncertainty_loss_der_square}')
            ...
            #Define threshold that is calculated by mean of standard deviations of train data for three sensors
            ...
            #Consider mean of loss,uncertainty_loss and p_values because output must have one output for there different sensors
            scored['loss'] = [np.mean(self.loss)]

            scored['threshold'] = self.threshold
            self.log_info(f"threshold:{scored['threshold']}")

            scored['upper_uncertainty_loss'] = np.mean([self.loss + self.uncertainty_loss.values])
            scored['below_uncertainty_loss'] = np.mean([self.loss - self.uncertainty_loss.values])
...
#custom_create_monitor_graph_raw_data could modify appearance of final graph that will be showed in output web page
...
#custom_create_monitor_graph_calculation could modify appearance of final graph that will be showed in output web page
def custom_create_monitor_graph_calculation(data, sender_agent):
    ...
    **kwargs
    ...
    all_go = [go.Scatter(x=x, y=loss, mode="lines", name='Loss', line=dict(color="#3399FF")),
              ...
              go.Scatter(x=x, y=upper_uncertainty_loss, mode="lines", name='Upper_uncertainty_loss',
                         line=dict(color="#CCE5FF")),
              go.Scatter(x=x, y=below_uncertainty_loss, mode="lines", name='Below_uncertainty_loss',
                         line=dict(color="#CCE5FF")),
...
def main():
    ...
    monitor_agent_2.init_parameters(
        plot_filter=['Time', 'loss', 'threshold', 'upper_uncertainty_loss', 'below_uncertainty_loss', 'p_values'],
